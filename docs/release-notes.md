...
An allow-listed host skips Gateway altogether when the URL is plain `http`: the browser can fetch that by itself and a proxy that hands it back unchanged buys nothing. Over `https` it still goes through the live proxy, because a browser this old left to do its own handshake against a 2026 server fails at it, which is the whole reason this program exists. The script is handed the URL as well as the host, so it decides per request rather than per host.
