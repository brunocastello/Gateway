...
| `allow_sslv3` | `1` | Inside `connect_mitm`, offer SSL 3.0 to a browser that offers nothing better. Netscape 3, IE 3 and IE 4 have no TLS at all and could previously reach nothing but `rewrite_https`; with this they get a real `https://` address bar, over RC4 (or export-grade RC2, for a browser that offers nothing else). BearSSL's server takes the highest version in common, so a browser that can manage TLS 1.0 still gets TLS 1.0 and never sees SSL 3.0. Set to `0` to refuse it — the handshake then fails as it did before, and `rewrite_https` still serves those browsers. |
